...
import { Bell, RotateCcw, CalendarIcon, Menu, Search, LayoutGrid, Moon, Sun, Star, Trash2, Edit2, X, Plus, ClipboardList, Users, Calendar, ChartPie, Layers } from 'lucide-react';
...
const LandingPage = () => {
    ...
    <div className={`fixed top-0 right-0 h-full w-full md:w-1/3 ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white'} shadow-lg transform transition-transform duration-300 ${showTaskDetail ? 'translate-x-0' : 'translate-x-full'} z-50`}>
      <div className="p-6 h-full overflow-y-auto">
        <button
          ...
          className={`absolute top-4 right-4 ${isDarkMode ? 'text-green-300 hover:text-gray-100' : 'text-gray-500 hover:text-gray-700'}`}
        >
          <X className="w-5 h-5" />
        </button>
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-white text-gray-900'}`}>
      {/* Header */}
      <header className={`p-4 ${isDarkMode ? 'bg-gray-800' : 'bg-green-200'} shadow-sm`}>
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center">
              ...
              <span className="ml-2 font-bold text-green-500 text-2xl">It</span>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-4">
            <div className="relative">
              <div className="flex items-center">
                ...
                <button
                  ...
                  className={`p-2 rounded-full ${isSearchOpen ? 'bg-green-600 text-white' : ''}`}
                >
            ...
            {isAuthenticated ? (
              <button
                ...
                className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-600"
              >
                Logout
              </button>
            ) : (
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-6 p-6">
        ...
        <aside className="w-full md:w-64 space-y-6">
          {/* Profile Block */}
          <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-green-200'} rounded-xl p-6 shadow-sm`}>
          ...
          {/* Add List Block */}
          <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-green-200'} rounded-xl p-6 shadow-sm`}>
            <button className="w-full flex items-center space-x-2 text-green-800 hover:text-green-600">
          ...
          <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-green-200'} rounded-xl p-6 shadow-sm`}>
            ...
            <div className="relative">
              <input
                ...
                className={`w-full p-2 pr-10 border-2 border-green-500 rounded-lg ${isDarkMode ? 'bg-gray-700 text-white' : 'bg-green-200 text-gray-900'
                  } focus:outline-none focus:ring-2 focus:ring-green-500`}
              />
        ...
        <main className="flex-1">
          <div className={`p-6 ${isDarkMode ? 'bg-gray-800' : 'bg-green-200'} rounded-xl shadow-sm mb-6`}>
          ...
          <div className={`p-6 ${isDarkMode ? 'bg-gray-800' : 'bg-green-200'} rounded-xl shadow-sm`}>
          {searchQuery && (
